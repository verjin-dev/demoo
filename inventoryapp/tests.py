...
class AppTest(TestCase):
    ...
    def test_4_inventory_post_error(self):
        ...
        print(res.json())
        assert b'item with this barcode already exists.' in res.content
    ...
    def test_6_inventory_get_query_category_success(self):
        # Create items
        self.client.post('/inventory/items/', data={'name': 'shirt', 'category': 'top wear', 'price': 700, 'discount': 20, 'quantity': 2, 'barcode': 123456})
        self.client.post('/inventory/items/', data={'name': 't-shirt', 'category': 'top wear', 'price': 1300, 'discount': 30, 'quantity': 2, 'barcode': 12345687})
        self.client.post('/inventory/items/', data={'name': 'shorts', 'category': 'bottom wear', 'price': 300, 'discount': 5, 'quantity': 3, 'barcode': 123455})
        
        # Query items by category
        response = self.client.get('/item/query/top%20wear/')
        ...
        assert 'shirt' in response_data[0]['name']
        assert 't-shirt' in response_data[1]['name']
        assert 200 == response.status_code
